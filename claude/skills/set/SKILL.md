...
Convert an existing project into the Leanforge **project harness** — the durable documentation layer
that every later agent (Leanforge or not) works inside. Set reads the codebase, elicits the
...
Set is a **one-time conversion**, not a task runner. It writes documentation only — it does
**not** create a 3-doc (that is `Prime`'s job) and does **not** execute code (that is `Run`'s).
After it finishes, clear the session before running `Prime` → `Run`: Set is an
...
- Invocation: the user invokes the `Set` skill, no arguments — Set reads the **current
  project**.
- **Existing codebase expected.** Set converts a project that already has code. For a
  greenfield project (no code yet), there is nothing to migrate — direct the user to `Prime` (which
  designs the project's first cycle and lets `Run` create the harness from scratch).
...
dependency, and gap. This is the **ledger** ELICIT works from (`references/set-elicit.md`): each
item must close as `confirmed` / `asked-answered` / `N/A — reason`, so coverage is *observable*, not
asserted.

## Phase 2 — ELICIT (collect what code can't reveal) — `references/set-elicit.md`

**Force-load `references/set-elicit.md`.** Using the SCAN map, ask the user for the
...
CLAUDE.md/AGENTS.md **critically** — decide what to fold into the Leanforge system, what to drop, and
