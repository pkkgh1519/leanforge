# Leanforge:Set-elicit.md — Phase 2 ELICIT (project-wide extraction)

Collect what the code cannot reveal — across the **whole project**, not a single task. This is
independent of `Prime`'s `elicitation.md`: that one is task-focused; this is project-wide, with its
...
**A thin / undocumented codebase RAISES the bar — it does not lower it.** Sparse code is Set's
...
harder.** A hollow harness from a sparse codebase is the exact failure Set exists to prevent.
...
Set's form of `Prime`'s decision-surface accounting: the SCAN manifest *is* the surface.)
...
any existing CLAUDE.md/AGENTS.md **critically**: decide what to fold into the Leanforge system, what
