...
task's* WHAT lives in `spec.md` (what `Run` implements); the Foundation is read as context (no
per-entity target/context tags). `Run` later turns the model into `business-rules.md`.
