...
- **Phase 1.2 observation study protocol:** 고정된 router revision과 사전 등록된 cohort마다 shadow prediction을 숨긴 상태에서 최종 3-doc·review·Run 증거로 observed class를 독립 판정하고, redacted manual worksheet로 exact·conservative·underclassification·unevaluable 결과를 기록한다. 완료 record는 공개 저장소에 커밋하지 않는다.
...
1. **Adaptive Assurance Phase 1.2 실행:** 고정된 router revision 하나에서 최소 35개의 usable real observation을 수집하되 shadow Lite 15건 이상, Standard 10건 이상, Assurance 10건 이상을 포함한다. Prediction 확인 전에 관측 구간·host 범위·포함 및 제외 기준을 고정하고 해당 구간의 모든 eligible cycle을 기록한다. 부재·제외·unevaluable record는 성공으로 세지 않고 별도 보고한다. Router·contract·predicate·grounding이 바뀌면 새 batch를 시작하고 이전 coverage를 이월하지 않는다. Claude Code와 Codex 모두가 pilot 범위라면 두 host 모두 usable 표본에 포함한다.
...
- Token·latency 개선을 주장할 필요가 생길 때만 100회 반복 측정을 사용한다. 반복 측정은 deterministic gate나 관측 연구의 독립 판정을 대체하지 않는다.
