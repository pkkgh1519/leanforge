...
- **Strict Lite alpha candidate:** 내부 activation 파일이 없으면 Full Assurance를 그대로 사용한다. 활성화된 qualifying delta에서만 Prime intent-completeness reviewer와 proven no-op harness sync를 생략하며, 3-doc gate·full verify·final reviewer·recovery·integration ownership은 유지한다.
