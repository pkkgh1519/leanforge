...
unknowns. This is one of Prime's two required independent intent/artifact audits — the other is the
earlier **intent-completeness** check. The optional ORIENT evidence-grounding scout is separate:
read-only evidence collection, not review and not an artifact or intent-completeness gate. Every
authoring stage runs inline because intent grounding needs raw context, not a summary (Core
principles).
...
Prime must never be authored to lean on it: an upstream that under-elicits "because the gate will
