...
```json
{
  ...
  "constraints": [
    ...
    "Single-risky work has exact cardinality and order: orchestrator-created worktree, implementer implementation, captured evidence, and verification, then orchestrator merge precondition, merge gate, and one task merge.",
    "Parallel work correlates at least two isolated task worktrees through each reached per-task implementation, verification, branch gate, and task merge stage by the same stable non-empty task identities; aggregate verification and gate results precede serial merge, regeneration, wiring, and a green integration gate.",
    "External work is performed by a base-pinned implementer without a task worktree or file diff.",
    "Every selected-route event before the first failed result is structurally validated even when the failure event is outside that route; the ordered reached prefix leaves later success-only stages optional and forbids downstream success-route advancement.",
...
```json
{
  ...
  "definition": "For each stable non-empty task_id, conditional spec review occurs if and only if exactly one non-contradictory risky-task fact and one downstream-cascade-risk fact apply to that same task; unrelated or contradictory task facts cannot combine to trigger it. Each conditional spec review is owned by a fresh independent reviewer leaf and never replaces final full-diff review after a clear conditional result. Every successful completion or user gate follows exactly one final full-diff review with outcome green and then exactly one review verdict with outcome clear. The final full-diff review and its verdict remain fresh-reviewer-owned, while completion and user-gate endpoints remain orchestrator-owned. Every review verdict requires exactly one applicable final full-diff review: clear correlates only to its unique green review, while blocking correlates only to its unique blocking review and remains a failed result that enters the runtime-owned failure overlay before continuation. Duplicate or conflicting verdicts are invalid.",
  "constraints": [
    ...
    "Each risky downstream cascade task requires exactly one fresh-reviewer-owned conditional spec review before dispatch and final review, while either missing prerequisite forbids it for that task.",
    "Conditional and final review events and review verdicts carry their actual closed result values; final full-diff review and review verdict remain fresh-reviewer-owned.",
    "Successful completion and the user gate require exactly one green final full-diff review, then exactly one clear review verdict, then an orchestrator-owned endpoint.",
    "Every verdict requires exactly one applicable final full-diff review; clear correlates only to a unique green final review, blocking correlates only to a unique blocking final review, and blocking enters the runtime-owned failure overlay before continuation."
  ]
}
```
