# graph-contract.md — the Execution Graph, from Run's side (parse contract)

Run does **not** compute dependencies — `Prime` (the producer) already did, against the whole
project. Run's job is to *parse* the graph faithfully and schedule from it. This file is the
schema Run reads against, so the parser has an authoritative reference instead of re-deriving the
shape from memory. The producer's authoring view is in its own `output-format.md` /
`dependency-calc.md` (in the producer's references, not Run's); this is the same contract, stated
for the consumer.
...
- **`tasks[].depends`** — the task ids that must finish first. This is the **only encoded
  judgment**. Run follows it and never re-judges, adds, drops, or reorders an edge.
- **`tasks[].risk`** *(OPTIONAL)* — `RISKY | MECHANICAL | NONE`. Run has **two** consumer-side uses:
...
- **`regen_barriers[]`** — `{ after: [ids], run: "<cmd>" }`: a cross-cutting step that must run
  **between** waves once `after` is satisfied (schema→client/type generation, contract→codegen,
  catalog rebuilds, …). The command is project-specific (discovered by the producer); Run runs it
...
## Rules Run must hold when reading the graph
