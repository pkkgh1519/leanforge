...
The verification criteria for a project harness. Used by **Set's REVIEW phase** and by
**`Run`'s final-review lenses 3–4** — one source of truth, so both judge the harness the same way.
...
- **Language** — the harness is written natively in the **language the user communicates in**
  ...
  dispatch — in both Set and Run the finished work is verified by an independent dispatched
...
- **Harness describes the project only (blocking)** — flag any file that describes itself (its state,
  ...
  Leanforge's coined vocabulary to label the doc system** ("Leanforge" anywhere; calling the docs "the
  harness"/"하네스", a section "Harness navigation", or naming "3-doc"/"delta"/"wave"). Section titles
  use the project's plain terms. It describes the project, not the document. ("Leanforge" is a reliable
...
- **Needs user intent** (a gap only the user can fill — a domain rule, a policy decision, a
  rationale not on the record) → do not invent it; raise it to the user (Set: ask in the user
  gate; Run: surface as a blocking finding the orchestrator escalates).

In `Run`'s final review, harness findings carry the same blocking/advisory split as code findings.
