...
Defines what **`Prime`** (the producer) outputs and **`Run` consumes**. This is the contract between
`Prime` and `Run`.
...
A fenced `yaml` block inside the plan. Run parses it for scheduling:
...
- `risk: RISKY | MECHANICAL | NONE` is an **optional** per-task atom alongside `depends`.
  ...
  verified or reviewed, and never touches gate topology. (Run may also read it to choose a single-task
...
2. **Guarantee** (runtime, Run): before merging a wave, detect changed-file
